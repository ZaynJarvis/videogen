...
function buildMcpDeliveryBlock(mcp, sourceImage, improvement) {
  ...
  if (sourceImage || referenceImages.length) {
    lines.push('  Identity fidelity rules: first download/open every source/reference image and identify the clearest face/front view as the primary identity reference. Preserve face geometry, eye spacing, nose, mouth, jaw/chin shape, hairline, skin texture, and body proportions before applying outfit or style changes. Identity fidelity is higher priority than making the result prettier. Do not beautify, idolize, average, or replace the person with a same-type generic character. If your imagegen tool cannot use the images as actual visual references, stop and report that instead of generating from text only. Before delivery, compare the generated face against the references; if it does not look like the same person, regenerate and do not deliver the failed image.');
  }
  if (kind === 'sheet') {
    ...
    lines.push(`  Steps for you, the agent: (1) download/open source-image and every URL under reference-images, then use YOUR OWN imagegen tool with those images attached as actual visual references; do not ask Studio MCP to generate and do not replace the image references with text-only descriptions. Treat source-image, reference-images, and improvement text as one unified character design brief, with face identity first and clothing/style transformation second. (2) For speed, prefer generating ONE ${rows}x${cols} contact sheet containing all zones in the exact order above, then crop it locally into the ${zones.length || rows * cols} zone images. (3) For each final crop, call the Studio MCP server at endpoint with Authorization: Bearer &lt;token&gt;, JSON-RPC tools/call name "upload_image" arguments { "image":"&lt;image data URL&gt;", "name":"&lt;zone id&gt;.jpg", "tag":"design" }, then call tools/call name "deliver_character_zone" arguments { "claim_token":"&lt;token&gt;", "zone_id":"&lt;zone id&gt;", "image_url":"&lt;uploaded image url&gt;" }. If local cropping is not available, upload the whole sheet with upload_image and call deliver_character_sheet arguments { "claim_token":"&lt;token&gt;", "sheet_image_url":"&lt;uploaded sheet url&gt;" }. Zone ids: [${escapeContextText(zoneIds, 600)}]. The token covers all zones until done and expires in 30 min.`);
  } else {
    ...
    lines.push(`  Steps for you, the agent: (1) download/open source-image and every URL under reference-images, then use YOUR OWN imagegen tool with those images attached as actual visual references; do not ask Studio MCP to generate and do not replace the image references with text-only descriptions. Treat source-image, reference-images, and improvement text as one unified single-zone brief, with face identity first and clothing/style transformation second. (2) Generate only this one requested zone image. (3) Call the Studio MCP server at endpoint with Authorization: Bearer &lt;token&gt;, JSON-RPC tools/call name "upload_image" arguments { "image":"&lt;image data URL&gt;", "name":"${escapeContextText(zoneId || '<zone id>', 100)}.jpg", "tag":"design" }. (4) Call tools/call name "deliver_character_zone" arguments { "claim_token":"&lt;token&gt;", ${zoneId ? `"zone_id":"${escapeContextText(zoneId, 80)}", ` : ''}"image_url":"&lt;uploaded image url&gt;" }. The token is single-use and expires in 30 min. Deliver exactly one final image.`);
  }
