...
function buildMcpDeliveryBlock(mcp, sourceImage, improvement) {
  ...
  if (kind === 'sheet') {
    ...
    lines.push(`  Steps for you, the agent: (1) download/open source-image and every URL under reference-images, then use YOUR OWN imagegen tool with those images attached as actual visual references; do not ask Studio MCP to generate and do not replace the image references with text-only descriptions. Treat source-image, reference-images, and improvement text as one unified single-zone brief. (2) Generate only this one requested zone image. (3) Call the Studio MCP server at endpoint with Authorization: Bearer &lt;token&gt;, JSON-RPC tools/call name "upload_image" arguments { "image":"&lt;image data URL&gt;", "name":"${escapeContextText(zoneId || '<zone id>', 100)}.jpg", "tag":"design" }. (4) Call tools/call name "deliver_character_zone" arguments { "claim_token":"&lt;token&gt;", ${zoneId ? `"zone_id":"${escapeContextText(zoneId, 80)}", ` : ''}"image_url":"&lt;uploaded image url&gt;" }. The token is single-use and expires in 30 min. Deliver exactly one final image.`);
  }
