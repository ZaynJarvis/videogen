...
function buildMcpDeliveryBlock(mcp, sourceImage, improvement) {
  ...
  if (sourceImage || referenceImages.length) {
    lines.push('  Rules: download/open all visual refs; use the clearest face as identity anchor; preserve face/body before style/outfit; no beautified or generic replacement. Use an isolated subagent/worker for imagegen if available. If image refs cannot be attached to imagegen, stop and report. Verify same person before delivery.');
  }
  if (kind === 'sheet') {
    ...
    lines.push(`  Zones ${rows}x${cols} order: [${escapeContextText(zoneIds, 600)}]. Meaning: full body front/left/back; face close-up front/3q-left/3q-right; smiling expression; waist-up; outfit 3q.`);
    lines.push(`  Steps: (1) in isolated imagegen context, use source-image + reference-images as real visual refs with improvement text. (2) Prefer one ${rows}x${cols} sheet in exact zone order, then crop ${zones.length || rows * cols} zones. (3) upload_image each crop as <zone id>.jpg tag=design, then deliver_character_zone. If crop is unavailable, upload whole sheet and deliver_character_sheet. Token expires in 30 min.`);
  } else {
    ...
    lines.push(`  Steps: (1) in isolated imagegen context, use source-image + reference-images as real visual refs with improvement text. (2) Generate only this zone. (3) upload_image name="${escapeContextText(zoneId || '<zone id>', 100)}.jpg" tag=design, then deliver_character_zone${zoneId ? ` zone_id="${escapeContextText(zoneId, 80)}"` : ''}. Token expires in 30 min.`);
  }
